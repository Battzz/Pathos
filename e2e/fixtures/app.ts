...
// Shared Playwright fixture that boots the Pathos shell past its first-run
// gates so specs start from the main workspace surface:
//   - stubs `get_app_settings` to mark onboarding completed so the overlay
//     never appears
//   - exposes `window.__PATHOS_E2E__` as a hook for specs to override
...
export const test = base.extend<PathosFixtures>({
	app: async ({ page }, use) => {
		await page.addInitScript(() => {
			window.__PATHOS_E2E__ = {
				invokeOverrides: {
