...
use pathos_lib::agents::{build_send_message_params, BuildSendMessageParamsInput};
use pathos_lib::data_dir;
use serde_json::Value;
use tempfile::TempDir;

/// Serialize intra-binary access to the process-wide `PATHOS_DATA_DIR`
...
/// RAII test env: takes the env-var lock, overrides `PATHOS_DATA_DIR`,
...
impl TestEnv {
    fn new() -> Self {
        ...
        pathos_lib::schema::ensure_schema(&conn).unwrap();
        conn.execute(
...
#[test]
fn omits_additional_directories_when_pathos_session_id_is_absent() {
