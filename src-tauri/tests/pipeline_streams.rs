...
use pathos_lib::pipeline::PipelineEmit;
use serde::Serialize;
...
fn part_type(part: &pathos_lib::pipeline::types::ExtendedMessagePart) -> &'static str {
    use pathos_lib::pipeline::types::{ExtendedMessagePart, MessagePart};
    match part {
...
fn stream_replay() {
    ...
    glob!("fixtures/streams/*/*.jsonl", |path| {
        ...
        if std::env::var("PATHOS_INVENTORY").is_err() {
            assert!(
                dropped.is_empty(),
