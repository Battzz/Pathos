...
impl SidecarProcess {
    ...
    fn start() -> Result<(Self, BufReader<std::process::ChildStdout>)> {
        ...
        if let Ok(dir) = crate::data_dir::logs_dir() {
            cmd.env("PATHOS_LOG_DIR", dir);
        }
        if let Ok(level) = std::env::var("PATHOS_LOG") {
            cmd.env("PATHOS_LOG", level);
        } else if crate::data_dir::is_dev() {
            cmd.env("PATHOS_LOG", "debug");
        }
        ...
        if !is_dev {
            ...
            if let Some(path) = bundled_paths.claude_cli {
                cmd.env("PATHOS_CLAUDE_CODE_CLI_PATH", &path);
            }
            if let Some(path) = bundled_paths.codex_bin {
                cmd.env("PATHOS_CODEX_BIN_PATH", &path);
            }
            if let Some(path) = bundled_paths.bun_bin {
                cmd.env("PATHOS_BUN_PATH", &path);
            }
        }
...
fn resolve_sidecar_path() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("PATHOS_SIDECAR_PATH") {
    ...
    //    (e.g. Pathos.app/Contents/MacOS/pathos-sidecar on macOS,
    //     C:\Program Files\Pathos\pathos-sidecar.exe on Windows).
    if let Ok(exe) = std::env::current_exe() {
        if let Some(exe_dir) = exe.parent() {
            // Windows: Tauri CLI emits externalBins with the same .exe suffix
            // as the parent Pathos binary. On Unix, no extension.
    ...
    bail!("Sidecar not found. In dev, ensure sidecar/src/index.ts exists. Set PATHOS_SIDECAR_PATH to override.")
}
...
mod tests {
    ...
    fn bundled_agent_paths_resolve_from_running_app() {
        ...
        let exe = root.path().join("Pathos.app/Contents/MacOS/Pathos");
        let resources = root.path().join("Pathos.app/Contents/Resources/vendor");
        ...
        assert_eq!(
            ...
            root.path()
                .join("Pathos.app/Contents/Resources/vendor/claude-code/cli.js")
        );
        assert_eq!(
            paths.codex_bin.unwrap(),
            root.path()
                .join("Pathos.app/Contents/Resources/vendor/codex/codex")
        );
        assert_eq!(
            paths.bun_bin.unwrap(),
            root.path()
                .join("Pathos.app/Contents/Resources/vendor/bun/bun")
        );
    }
}
