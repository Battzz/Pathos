...
/// test at a time touches the `PATHOS_DATA_DIR` env var.
...
impl TestEnv {
    pub fn new(name: &str) -> Self {
        ...
        let root = env::temp_dir().join(format!("pathos-test-{name}-{}", Uuid::new_v4()));
        env::set_var("PATHOS_DATA_DIR", root.display().to_string());
...
impl Drop for TestEnv {
    fn drop(&mut self) {
        env::remove_var("PATHOS_DATA_DIR");
...
impl GitTestRepo {
    ...
    pub fn init() -> Self {
        ...
        Self::run_git(p, &["config", "user.email", "test@pathos.test"]);
