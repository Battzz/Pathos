...
pub(super) fn stream_via_sidecar(
    ...
    tracing::debug!(
        ...
        pathos_session_id = ?request.pathos_session_id,
        provider = %model.provider,
        "Session resume context"
    );

    let pathos_session_id = request.pathos_session_id.clone();
    let sidecar_session_id = pathos_session_id
        .clone()
    ...
    let hsid_copy = pathos_session_id;
    ...
    tauri::async_runtime::spawn_blocking(move || {
            ...
            match crate::models::db::write_conn() {
                Ok(conn) => {
                    if let Err(e) = conn.execute(
                        ...
                        rusqlite::params![fast_mode, &ctx.pathos_session_id],
                    ) {
        ...
        let mut turn_session = state::TurnSession::new(state::TurnContext {
            ...
            fast_mode,
            pathos_session_id: hsid_copy.clone(),
                    ...
                    if should_adopt_provider_session_id(
                            ...
                            (&exchange_ctx, &crate::models::db::write_conn().ok())
                        {
                            if let Err(error) = conn.execute(
                                ...
                                params![ctx.pathos_session_id, sid, ctx.model_provider],
                            ) {
