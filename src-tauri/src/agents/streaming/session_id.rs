...
/// - It does not echo the pathos session id (defensive — sidecar should
...
pub(super) fn should_adopt_provider_session_id(
    ...
    pathos_session_id: Option<&str>,
) -> bool {
    !observed_provider_session_id.is_empty()
        && pathos_session_id != Some(observed_provider_session_id)
...
mod tests {
    ...
    #[test]
    fn provider_session_id_rejects_empty_and_pathos_echo_values() {
        assert!(!should_adopt_provider_session_id(None, "", None));
        assert!(!should_adopt_provider_session_id(
            None,
            "pathos-session-1",
            Some("pathos-session-1"),
        ));
    }
}
