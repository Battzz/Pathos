...
pub struct BuildSendMessageParamsInput<'a> {
    ...
    pub resume_session_at: Option<&'a str>,
    pub provider: &'a str,
...
pub fn build_send_message_params(input: BuildSendMessageParamsInput<'_>) -> Value {
    let additional_directories = lookup_workspace_linked_directories(input.pathos_session_id);
    ...
    let mut params = serde_json::json!({
        ...
        "resumeSessionAt": input.resume_session_at,
...
/// Load the workspace's `/add-dir` list via the pathos session id. Returns
...
pub fn lookup_workspace_linked_directories(pathos_session_id: Option<&str>) -> Vec<String> {
    let Some(hsid) = pathos_session_id else {
        return Vec::new();
    };
...
mod tests {
    ...
    fn with_test_db<F: FnOnce(&rusqlite::Connection)>(name: &str, f: F) {
        ...
        std::env::set_var("PATHOS_DATA_DIR", dir.path());
