...
impl BranchSwitchTestHarness {
    pub(crate) fn new() -> Self {
        ...
        run_in_repo(&source_repo, &["config", "user.name", "Pathos"]);
...
fn init_branch_switch_repo(repo: &Path) {
    ...
    run_in_repo(repo, &["config", "user.email", "pathos@example.com"]);
    run_in_repo(repo, &["config", "user.name", "Pathos"]);
