//! Clap argument definitions for every installed Pathos CLI subcommand.
//!
...
#[command(
    name = "pathos",
    version,
    about = "Pathos workspace, session, and agent CLI",
    long_about = "Remote-control Pathos from the terminal. Works against the same SQLite \
...
pub enum Commands {
    ...
    /// Report whether the current Pathos CLI entrypoint is installed to PATH and which data mode it uses.
    CliStatus,
    /// Ask a running Pathos app to quit (noop when it isn't running).
    Quit,
...
pub enum RepoAction {
    ...
    /// Change the default branch saved in the Pathos DB.
    DefaultBranch {
        #[arg(name = "ref")]
        repo_ref: String,
        branch: String,
