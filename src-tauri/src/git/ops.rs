...
/// no corresponding remote ref Pathos can see — treat as "branch was
/// never published".
...
pub fn preflight_merge_ref(workspace_dir: &Path, target_ref: &str) -> Result<MergePreflightResult> {
    ...
    let preflight_dir =
        std::env::temp_dir().join(format!("pathos-merge-preflight-{}", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        run(dir.path(), &["config", "user.email", "pathos@example.com"]);
        run(dir.path(), &["config", "user.name", "Pathos Test"]);
    ...
    fn init_repo_with_remote() -> (tempfile::TempDir, tempfile::TempDir) {
        ...
        run(
            clone_dir.path(),
            &["config", "user.email", "pathos@example.com"],
        );
        run(clone_dir.path(), &["config", "user.name", "Pathos Test"]);
