...
impl TestDataDir {
    pub(super) fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("pathos-test-{name}-{}", Uuid::new_v4()));
...
impl EditorFilesHarness {
    pub(super) fn new() -> Self {
        ...
        connection
            .execute(
                "INSERT INTO repos (id, name, root_path) VALUES ('repo-1', 'pathos', ?1)",
        ...
        let workspace_dir = crate::data_dir::workspace_dir("pathos", "editor-mode").unwrap();
...
impl GitRepoHarness {
    pub(super) fn new() -> Self {
        ...
        git_ops::run_git(["config", "user.email", "test@pathos.test"], Some(&root)).unwrap();
