...
//!
//! Level defaults:
//! - dev: `warn,pathos_lib=debug,pathos=debug` — app crates debug, deps warn.
//!   Keeps hyper/reqwest/rustls/h2 connection traces out of the stream.
//! - release: `info`.
//!
//! Override with `PATHOS_LOG`. Accepts either a bare level (`debug`, `info`, ...)
//! or a full `EnvFilter` directive list (e.g. `info,pathos_lib=trace,hyper=debug`).
...
const DEV_DEFAULT_DIRECTIVES: &str = "warn,pathos_lib=debug,pathos=debug";
...
/// need to pass it to the sidecar via `PATHOS_LOG_DIR`.
...
/// Build a fresh `EnvFilter`. `PATHOS_LOG` wins when set and parses; otherwise
...
mod tests {
    ...
    #[test]
    fn dev_default_allows_pathos_debug_but_caps_deps_at_warn() {
        // Dev default: pathos crates DEBUG, everything else WARN.
