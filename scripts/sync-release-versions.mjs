...
// Cargo.lock keeps a `[[package]]` entry for the pathos crate with its own
...
const cargoLockPattern = /(^name = "pathos"\nversion = )"[^"]*"/m;
if (!cargoLockPattern.test(cargoLock)) {
	throw new Error(
		'src-tauri/Cargo.lock is missing the `name = "pathos"` package entry',
	);
}
