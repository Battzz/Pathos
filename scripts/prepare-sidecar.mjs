 ...
 * 2. `bun run build` — produces `sidecar/dist/pathos-sidecar` plus the
 *    `sidecar/dist/vendor/` tree that Tauri bundles as resources.
 * 3. `cargo build --bin pathos-cli --release --target <triple>` — produces
...
function main() {
	...
	const sidecarSource = resolve(sidecarDir, "dist", "pathos-sidecar");
	const sidecarDestination = resolve(
		sidecarDir,
		"dist",
		`pathos-sidecar-${triple}`,
	);
	const cliBinaryName =
		process.platform === "win32" ? "pathos-cli.exe" : "pathos-cli";
	...
	const cliDestination = resolve(bundledBinDir, `pathos-cli-${triple}`);
	...
	// checkout can compile `pathos-cli` without depending on stale artifacts.
	copyFileSync(sidecarSource, sidecarDestination);

	run(
		`cargo build --manifest-path ${resolve(srcTauriDir, "Cargo.toml")} --bin pathos-cli --release --target ${triple}`,
		repoRoot,
	);
