...
describe("CliInstallPanel", () => {
	...
	it("renders the managed install state", async () => {
		...
		expect(screen.getByText("pathos-dev")).toBeInTheDocument();
		expect(screen.getByText("/usr/local/bin/pathos-dev")).toBeInTheDocument();
		expect(
	...
	it("renders the stale install state and allows reinstall", async () => {
		...
		apiMocks.getCliStatus.mockResolvedValue({
			installed: true,
			installPath: "/usr/local/bin/pathos",
			buildMode: "release",
			installState: "stale",
		});
