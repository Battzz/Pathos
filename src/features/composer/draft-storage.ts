...
const STORAGE_PREFIX = "pathos:composer-draft:";
...
export function savePersistedDraft(
	...
	try {
		...
		console.error(`[pathos] composer draft save failed for "${key}"`, error);
	}
}
...
export function clearPersistedDraft(contextKey: string): void {
	...
	try {
		...
		console.error(`[pathos] composer draft clear failed for "${key}"`, error);
	}
}
