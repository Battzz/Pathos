// Minimum set of IPC command responses needed to boot the Pathos React shell
...
export async function runInvoke(
	...
	const overrides = globalThis.window?.__PATHOS_E2E__?.invokeOverrides;
	...
	if (!handler) {
		...
		console.warn(`[pathos-e2e] unstubbed invoke: ${command}`, args);
		return undefined;
	}
