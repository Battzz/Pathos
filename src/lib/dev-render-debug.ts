...
declare global {
	interface Window {
		__PATHOS_DEV_RENDER_STATS__?: DevRenderStats;
	}
}
...
function ensureStats(): DevRenderStats | null {
	...
	if (!window.__PATHOS_DEV_RENDER_STATS__) {
		window.__PATHOS_DEV_RENDER_STATS__ = {
			composer: {
				rendersByContext: {},
	...
	return window.__PATHOS_DEV_RENDER_STATS__;
}
