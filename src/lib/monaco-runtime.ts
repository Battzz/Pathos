...
function themeId(theme: EditorTheme): string {
	return theme === "dark" ? "pathos-editor-dark" : "pathos-editor-light";
}
...
export async function createDiffEditor(options: {
	...
	const originalUri = monaco.Uri.file(options.path).with({
		query: "pathos-review=original",
	});
	const modifiedUri = monaco.Uri.file(options.path).with({
		query: "pathos-review=modified",
	});
...
function installEditorTheme(monaco: MonacoModule) {
	monaco.editor.defineTheme("pathos-editor-dark", {
		base: "vs-dark",
		inherit: true,
		rules: [
	...
	monaco.editor.defineTheme("pathos-editor-light", {
		base: "vs",
		inherit: true,
		rules: [
