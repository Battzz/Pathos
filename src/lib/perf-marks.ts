 ...
 *
 * All measure names are prefixed with `pathos:` so they are trivial to
 ...
 * `pathos:` automatically — pass the bare names you used with `mark()`.
 */
 ...
 * `pathos:<name>` PerformanceMeasure entry with optional `detail` payload
...
/**
 * Reset all `pathos:*` performance entries. Useful between scenarios in a
...
/**
 * Read all currently-recorded `pathos:*` measures. Returned in start-time
