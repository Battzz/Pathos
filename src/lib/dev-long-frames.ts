/**
 * Long-frame / FPS telemetry for Pathos.
 *
 ...
 *      of truth for the "Pathos 60→40 fps drop" the user is hunting.
 *
 * Both paths feed the same in-memory ring buffer and the same on-screen HUD.
 *
 * Gating: enabled iff `?perfHud=1` is in the URL OR
 * `VITE_PATHOS_PERF_HUD=1` is set at build time. The flag is shared with
...
function isEnabled(): boolean {
	...
	const envFlag = import.meta.env.VITE_PATHOS_PERF_HUD === "1";
	let queryFlag = false;
	try {
		queryFlag =
...
export function initDevLongFrames(): void {
	...
	window.__PATHOS_LONG_FRAMES__ = {
		...
		downloadJson: () => {
			if (typeof window === "undefined") return;
			const blob = new Blob([window.__PATHOS_LONG_FRAMES__!.dumpJson()], {
				type: "application/json",
			});
			const url = URL.createObjectURL(blob);
			const a = document.createElement("a");
			a.href = url;
			a.download = `pathos-long-frames-${Date.now()}.json`;
...
export function teardownDevLongFrames(): void {
	...
	hudCleanup?.();
	hudCleanup = null;
	delete window.__PATHOS_LONG_FRAMES__;
}
