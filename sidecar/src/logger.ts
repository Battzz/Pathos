...
class Logger {
	...
	constructor() {
		const envLevel = process.env.PATHOS_LOG?.toLowerCase();
		const level: Level =
		...
		// shells inheriting PATHOS_LOG_DIR from a release pathos would otherwise
		// pollute ~/pathos/logs. Bun auto-sets NODE_ENV=test.
		const isTest =
			process.env.NODE_ENV === "test" || process.env.BUN_TEST === "1";
		const logDir = process.env.PATHOS_LOG_DIR;
		if (logDir && !isTest) {
